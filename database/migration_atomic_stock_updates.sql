-- 1. Add top-level total_sold tracking to products table
ALTER TABLE public.products ADD COLUMN IF NOT EXISTS total_sold INTEGER DEFAULT 0;

-- 2. Create the RPC function for atomic order processing
-- This function runs as SECURITY DEFINER to bypass RLS UPDATE policy for shoppers
...
BEGIN
    ...
    IF v_variants IS NOT NULL AND jsonb_array_length(v_variants) > 0 THEN
        -- Rebuild the variants array with updated stock and sold counts
                ...
                CASE 
                    WHEN (obj->>'quantity') = p_variant_label THEN 
                        obj || jsonb_build_object(
                            ...
                            'total_sold', (COALESCE((obj->>'total_sold')::int, 0) + p_qty)
                        )
                    ELSE obj 
                END as new_obj
...
-- 3. Grant execute permission to everyone (since it's a public sales page)
