-- Add new columns if they don't exist (Idempotent check)
DO $$ 
BEGIN 
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='products' AND column_name='nutrition_info') THEN 
        ALTER TABLE public.products ADD COLUMN nutrition_info JSONB DEFAULT '{}'::jsonb; 
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='products' AND column_name='serving_suggestion') THEN 
        ALTER TABLE public.products ADD COLUMN serving_suggestion TEXT; 
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='products' AND column_name='product_name_telugu') THEN 
        ALTER TABLE public.products ADD COLUMN product_name_telugu TEXT; 
    END IF;
END $$;
